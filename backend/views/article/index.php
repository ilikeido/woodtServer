...
$modelLabel = new \backend\models\Article();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'article-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('article/index')]); ?>     
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "article-form", "class"=>"form-horizontal", "action"=>Url::toRoute("article/save")]); ?>                      
                 
          <input type="hidden" class="form-control" id="id" name="Article[id]" />
          ...
          <div id="title_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="title" name="Article[title]" placeholder="必填" />
              </div>
          ...
          <div id="description_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="description" name="Article[description]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>

          <div id="cover_id_div" class="form-group">
              <label for="cover_id" class="col-sm-2 control-label"><?php echo $modelLabel->getAttributeLabel("cover_id")?></label>
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="cover_id" name="Article[cover_id]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="view_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="view" name="Article[view]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="create_time_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="create_time" name="Article[create_time]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="cover_thumb_url_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="cover_thumb_url" name="Article[cover_thumb_url]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="parse_content_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="parse_content" name="Article[parse_content]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
...
function orderby(field, op){
 ...
 function initEditSystemModule(data, type){
	...
	if(type == "view"){
      ...
      $("#cover_id").attr({readonly:true,disabled:true});
      $("#view").attr({readonly:true,disabled:true});
      $("#create_time").attr({readonly:true,disabled:true});
      $("#cover_thumb_url").attr({readonly:true,disabled:true});
	...
	$('#edit_dialog_ok').addClass('hidden');
      ...
      $("#cover_id").attr({readonly:false,disabled:false});
      $("#view").attr({readonly:false,disabled:false});
      $("#create_time").attr({readonly:false,disabled:false});
      $("#cover_thumb_url").attr({readonly:false,disabled:false});
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('article/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#article-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('article/create')?>" : "<?=Url::toRoute('article/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
		data:{id:id},
