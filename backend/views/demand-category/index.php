...
$modelLabel = new \backend\models\DemandCategory();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'demand-category-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('demand-category/index')]); ?>     
            ...
            foreach ($models as $model) {
				...
				echo '      <a id="group_btn" class="btn btn-primary btn-sm" href="../demand-group/index?id='. $model->id .'"> <i class="glyphicon glyphicon-zoom-in icon-white"></i>分组</a>';
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "demand-category-form", "class"=>"form-horizontal", "action"=>Url::toRoute("demand-category/save")]); ?>                      
                 
          <input type="hidden" class="form-control" id="id" name="DemandCategory[id]" />
          ...
          <div id="name_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="name" name="DemandCategory[name]" placeholder="必填" />
              </div>
          ...
          <div id="title_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="title" name="DemandCategory[title]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="sort_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="sort" name="DemandCategory[sort]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="flag_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="flag" name="DemandCategory[flag]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
...
function orderby(field, op){
 ...
 function searchAction(){
		$('#demand-category-search-form').submit();
	}
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('demand-category/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('demand-category/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#demand-category-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('demand-category/create')?>" : "<?=Url::toRoute('demand-category/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
		data:{id:id},
