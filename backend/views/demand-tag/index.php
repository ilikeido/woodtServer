...
$modelLabel = new \backend\models\DemandTag();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'demand-tag-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('demand-tag/index')]); ?>     
                  ...
                  <div class="form-group" style="margin: 5px;">
                      ...
                      <input type="text" class="form-control" id="query[group_id]" name="query[group_id]"  value="<?=isset($query["group_id"]) ? $query["group_id"] : "" ?>">
                  </div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "demand-tag-form", "class"=>"form-horizontal", "action"=>Url::toRoute("demand-tag/save")]); ?>                      
                 
          <input type="hidden" class="form-control" id="id" name="DemandTag[id]" />

          <input type="hidden" class="form-control" id="group_id" name="DemandTag[group_id]" value="<?=$group_id?>"/>
          ...
          <div id="name_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="name" name="DemandTag[name]" placeholder="必填" />
              </div>
          ...
          <div id="flag_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="flag" name="DemandTag[flag]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="sort_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="sort" name="DemandTag[sort]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('demand-tag/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('demand-tag/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#demand-tag-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('demand-tag/create')?>" : "<?=Url::toRoute('demand-tag/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
		data:{id:id},
